...
###------------------------------------------------------
#      GWAS for microbiome genus data
###------------------------------------------------------
  ...
  for (j in 1:nsample)
  {
    ...
    for (k in 1:ntreatment)
    {
      ...
      for (l in 1:nasv)
      {
        ...
        cat("GWAS for",microbiometype[i],sampletype[j],treatmentname[k],"genus",l,"completed\n")
      } 
      
      write.table(pval_gwas,paste0("Result_GWAS_",microbiometype[i],"_genus_",sampletype[j],"_",treatmentname[k],".txt"),quote=FALSE)
