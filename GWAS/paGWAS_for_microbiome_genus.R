...
for (i in 1:nmicro)
{
  for (j in 1:nsample)
  {
    rawdta <- read.table(paste0(datapwd,microbiometype[i],"_genus_data_",sampletype[j],".txt"),header=TRUE)
    ...
    for (k in 1:ntreatment)
    {
      ...
      write.table(phdata_pa,paste0("PAdata_",microbiometype[i],"_genus_",sampletype[j],"_",treatmentname[k],".txt"),quote=FALSE)
    } # end loop k
...
for (i in 1:nmicro)
{
  for (j in 1:nsample)
  {
    ...
    for (k in 1:ntreatment)
    {
      PHdata <- read.table(paste0("PAdata_",microbiometype[i],"_genus_",sampletype[j],"_",treatmentname[k],".txt"),header=TRUE)
      ...
      write.table(res,paste0("result_PAGWAS_",microbiometype[i],"_genus_",sampletype[j],"_",
