...
for (i in 1:nmicro)
{
  for (j in 1:nsample)
  {
    rawdta <- read.table(paste0(datapwd,microbiometype[i],"_family_data_",sampletype[j],".txt"),header=TRUE)
    ...
    for (k in 1:ntreatment)
    {
      ...
      write.table(phdata_pa,paste0("PAdata_",microbiometype[i],"_family_",sampletype[j],"_",treatmentname[k],".txt"),quote=FALSE)
    } # end loop k
...
for (i in 1:nmicro)
{
  for (j in 1:nsample)
  {
    ...
    for (k in 1:ntreatment)
    {
      PHdata <- read.table(paste0("PAdata_",microbiometype[i],"_family_",sampletype[j],"_",treatmentname[k],".txt"),header=TRUE)
