  ...
  for (j in 1:nsample)
  {
    ...
    for (k in 1:ntreatment)
    {
      ...
      for (l in 1:nasv)
      {
        ...
        cat("GWAS for",microbiometype[i],sampletype[j],treatmentname[k],"asv",l,"completed\n")
      } 
    
      write.table(pval_gwas,paste0("Result_GWAS_",microbiometype[i],"_asv_",sampletype[j],"_",treatmentname[k],".txt"),quote=FALSE)
