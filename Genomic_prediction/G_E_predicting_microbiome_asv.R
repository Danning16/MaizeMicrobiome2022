...
###----------------------------------------------------------------------------------
#      Prediction for microbiome ASVs using genomic data and environmental characters
###----------------------------------------------------------------------------------
...
for (i in 1:nmicro)
{
  for (j in 1:nsample)
  {
    ...
    {
      PHdata <- read.table(paste0(datapwd,"BLUE_",microbiometype[i],"_asv_",sampletype[j],"_",treatmentname[k],".txt"),header=TRUE)
      ...
      for (l in 1:nasv)
      {
        ...
        for (m in 1:ngen)
        {
          ...
          fit.gblup <- BGLR(yNA,
                            ...
                            burnIn = 2000,
                            verbose = FALSE,
                            saveAt = paste0("GP_GBLUP_",microbiometype[i],"_asv_",sampletype[j],"_",treatmentname[k]))
      ...
      write.table(res,paste0("result_GP_GBLUP_",microbiometype[i],"_asv_",sampletype[j],"_",treatmentname[k],".txt"))
...
for (i in 1:nmicro)
{
  for (j in 1:nsample)
  {
    ...
    for (k in 1:ntreatment)
    {
      ...
      for (l in 1:nasv)
      {
        ...
        for (m in 1:ngen)
        {
          ...
          fit.gblup <- BGLR(yNA,
                            ...
                            burnIn = 2000,
                            verbose = FALSE,
                            saveAt = paste0("GP_ECBLUP_",microbiometype[i],"_asv_",sampletype[j],"_",treatmentname[k]))
          ypred[m] <- fit.gblup$yHat[m]
        }
      ...
      write.table(res_2,paste0("result_GP_ECBLUP_",microbiometype[i],"_asv_",sampletype[j],"_",treatmentname[k],".txt"))
...
for (i in 1:nmicro)
{
  for (j in 1:nsample)
  {
    ...
    for (k in 1:ntreatment)
    {
      ...
      for (l in 1:nasv)
      {
        ...
        for (m in 1:ngen)
        {
          ...
          fit.gblup <- BGLR(yNA,
                            ...
                            burnIn = 2000,
                            verbose = FALSE,
                            saveAt = paste0("GP_ECGBLUP_",microbiometype[i],"_asv_",sampletype[j],"_",treatmentname[k]))
          ypred[m] <- fit.gblup$yHat[m]
        }
      ...
      write.table(res_3,paste0("result_GP_ECGBLUP_",microbiometype[i],"_asv_",sampletype[j],"_",treatmentname[k],".txt"))
